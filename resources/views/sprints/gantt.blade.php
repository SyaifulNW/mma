...
<script>
    ...
    dataTable.addColumn({ type: 'string', role: 'tooltip', p: { html: true } }); // <-- tooltip custom

    // Data sprint dari backend
    const sprintData = @json($sprints);

    sprintData.forEach(s => {
        if (s.mulai && s.selesai) {
            let color = '#999';
            if (s.status === 'pending') color = '#e74c3c';   // merah
            if (s.status === 'progress') color = '#f1c40f';  // kuning
